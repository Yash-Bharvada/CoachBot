...
class Settings(BaseSettings):
    ...
    tavus_workspace_id: str | None = Field(
        default=None,
        description="Tavus workspace ID used for API context and recording organization.",
    )
    tavus_pal_id: str | None = Field(
        default=None,
        description="Tavus PAL (Persona + Avatar Link) ID — a combined persona+face identifier.",
    )
    tavus_persona_id: str | None = Field(
        default=None,
        description="Default Tavus persona_id used when creating conversations. Overrides PAL-derived persona when set.",
    )
    tavus_face_id: str | None = Field(
        default=None,
        description="Default Tavus face_id used when creating conversations. Overrides PAL-derived face when set.",
    )
    tavus_replica_id: str | None = Field(
        default="r79e883a",
    ...
    cors_allow_credentials: bool = Field(
        default=True,
        description="Whether cookies/authorization headers are allowed across origins.",
    )
    frontend_base_url: str = Field(
        default="http://localhost:3000",
        description=(
            "Public base URL of the frontend, used when building absolute "
            "callback URLs (e.g. Tavus webhook redirects) that point back to the app."
        ),
    )
